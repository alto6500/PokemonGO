...
public class PokemonGo {
    ...
    private static void addAllOptions(MenuDaw menu) {
        try {
            ...
            menu.addOption("Eliminar Pokemon de la BD");
    ...
    private void eliminarPokemon() {
        try {
            sc = new Scanner(System.in);
            System.out.println("Pon el nombre del Pokemon a descartar");
            String nombre = sc.nextLine();
            Pokemon descartado = pokedex.esborrarPokemon(nombre);
            if (descartado!=null)
            {
                System.out.println("Borrado " + descartado);
            }
            else
            {
                System.out.println("El Pokemon " + nombre + " ya no existe en la BD ");
            }
        } catch (SQLException ex) {
            Logger.getLogger(PokemonGo.class.getName()).log(Level.SEVERE, null, ex);
        }
